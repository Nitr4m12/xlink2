...
const ResourceAccessorELink& AssetExecutorELink::getResourceAccessor_() const 
{
    return static_cast<UserInstanceELink*>(mpUserInstance)->getResourceAccessor();
}
...
{
    if (mpAssetCallTable) {
        if (getResourceAccessor_().isFollow(*mpAssetCallTable))
            mBitFlag.setBit(0);
    }
}
...
{
    auto& accessor{getResourceAccessor_()};
